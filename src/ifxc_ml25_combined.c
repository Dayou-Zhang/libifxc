...
static int
ifxc_ml25_prepare_component_major_point(
...
ifxc_status
ifxc_ml25_combined_eval_contracted_component_major(
    const ifxc_mgga_func_type *p,
    size_t npoints,
    const double *rho,
    const double *sigma,
    ...
    double *d_tau)
{
  ...
  for(point = 0; point < npoints; ++point){
    ...
    if(!ifxc_ml25_prepare_component_major_point(
    ...
    if(p->nspin == IFXC_MGGA_UNPOLARIZED){
      ml25_combined_func_vxc_unpol(
          p, 0, local_rho, local_sigma, local_lapl, local_tau, &out);
    }else{
      ml25_combined_func_vxc_pol(
          p, 0, local_rho, local_sigma, local_lapl, local_tau, &out);
    }
...
ifxc_status
ifxc_ml25_combined_integral_order0_component_major(
    const ifxc_mgga_func_type *p,
    size_t npoints,
    const double *rho,
    const double *sigma,
    ...
    double *features)
{
  ...
  memset(features, 0, IFXC_ML25_NFEATURES * sizeof(double));
  if(npoints == 0){
    return IFXC_OK;
  }
  if(rho == NULL || sigma == NULL || tau == NULL || weights == NULL){
    return IFXC_E_INVALID_ARGUMENT;
  }
  ...
  for(point = 0; point < npoints; ++point){
    ...
    double local_tau[2] = {0.0, 0.0};
    double local_lapl[2] = {0.0, 0.0};
    double local_features[IFXC_ML25_NFEATURES] = {0.0};
    ifxc_mgga_out_params out = {0};
    double density;
    size_t feature;
    ...
    out.zk = local_features;
    if(p->nspin == IFXC_MGGA_UNPOLARIZED){
      ml25_combined_func_exc_unpol(
          p, 0, local_rho, local_sigma, local_lapl, local_tau, &out);
      density = rho[point];
    }else{
      ml25_combined_func_exc_pol(
          p, 0, local_rho, local_sigma, local_lapl, local_tau, &out);
      density = rho[point] + rho[npoints + point];
    }
    for(feature = 0; feature < IFXC_ML25_NFEATURES; ++feature){
      features[feature] += weights[point] * density * local_features[feature];
    }
  }
