...
#define XC_MGGA_XC_ML25        888 /* Minnesota ML25 exchange-correlation functional */

typedef struct {
  int id;
  xc_mgga_funcs_variants *work;
} mgga_xc_ml25_params;

#define N_PAR 1
static const char  *param_names[N_PAR]  = {
  "id"
};
static const char  *param_desc[N_PAR]   = {
  "Integer ID of the integral to compute"
};
static const double ml25_params[N_PAR] = {0.0};
...
static xc_mgga_funcs_variants *work_mgga_ml25[N_INT + 1] =
{
    &mn15_work_mgga,
    &lak_x_work_mgga,
    &lak_c_work_mgga,
...
#define DEFINE_WORK_MGGA(order, spin, n) \
static void \
work_mgga_ ## order ## _ ## spin \
(const struct xc_func_type *p, size_t np, \
 const double *rho, const double *sigma, const double *lapl, const double *tau, \
 xc_mgga_out_params *out) { \
    ((mgga_xc_ml25_params *)p->params)->work-> spin[n](p, np, rho, sigma, lapl, tau, out); \
}

DEFINE_WORK_MGGA(exc, unpol, 0)
DEFINE_WORK_MGGA(vxc, unpol, 1)
DEFINE_WORK_MGGA(fxc, unpol, 2)
DEFINE_WORK_MGGA(kxc, unpol, 3)
DEFINE_WORK_MGGA(lxc, unpol, 4)
DEFINE_WORK_MGGA(exc, pol, 0)
DEFINE_WORK_MGGA(vxc, pol, 1)
DEFINE_WORK_MGGA(fxc, pol, 2)
DEFINE_WORK_MGGA(kxc, pol, 3)
DEFINE_WORK_MGGA(lxc, pol, 4)
...
static void
ml25_set_param(struct xc_func_type *p, const double *ext_params) {
  int id = (int)(*ext_params + 0.5);
  assert(id >= 0 && id <= N_INT);
  p->ext_params[0] = id;
  mgga_xc_ml25_params *params = p->params;
  params->id = id;
  params->work = work_mgga_ml25[id];
}

static void
mgga_xc_ml25_init(xc_func_type *p)
{
  assert(p!=NULL && p->params == NULL);
  p->params = libxc_malloc(sizeof(mgga_xc_ml25_params));
}

#ifdef __cplusplus
extern "C"
#endif
const xc_func_info_type xc_func_info_mgga_xc_ml25 = {
  XC_MGGA_XC_ML25,
  XC_EXCHANGE_CORRELATION,
  "Minnesota ML25 exchange-correlation functional",
  XC_FAMILY_MGGA,
  {NULL, NULL, NULL, NULL, NULL}, /* TODO: reference */
  XC_FLAGS_3D | XC_FLAGS_NEEDS_TAU | MAPLE2C_FLAGS,
  1e-15,
  {1, param_names, param_desc, ml25_params, ml25_set_param},
  mgga_xc_ml25_init, NULL,
  NULL, NULL, &work_mgga
};
